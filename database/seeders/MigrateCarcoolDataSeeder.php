...
class MigrateCarcoolDataSeeder extends Seeder
{
  ...
  protected $ordenTrabajoMap = [];
    ...
    {
        $this->command->info('🚀 Iniciando migración de datos desde base de datos fuente...');

        // Usar conexión fuente configurada en config/database.php
        // Las credenciales se configuran vía variables de entorno:
        // DB_SOURCE_HOST, DB_SOURCE_PORT, DB_SOURCE_DATABASE, DB_SOURCE_USERNAME, DB_SOURCE_PASSWORD
        $this->sourceConnection = DB::connection('mysql_source');
        
        // Verificar conexión
        try {
            $testCount = $this->sourceConnection->table('customers')->count();
            $this->command->info("✓ Conexión establecida. {$testCount} clientes en base de datos fuente.");
        } catch (\Exception $e) {
            $this->command->error('❌ No se pudo conectar a la base de datos fuente.');
            $this->command->error('   Verifica las variables de entorno: DB_SOURCE_HOST, DB_SOURCE_DATABASE, DB_SOURCE_USERNAME, DB_SOURCE_PASSWORD');
